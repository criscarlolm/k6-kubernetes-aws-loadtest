...
export function getBaseUrl(environment) {
    const environments = {
        aws: {
            baseUrl: 'https://api1.aws.legalmatch.com'
        }
    };
     ...
     caseNumber
        ...
        variables: {
            data: {
                ...
                city: 'Nowheresville',
                client: {
                    firstName: 'Introthreee',
                    lastName: 'Clientthree',
                    email: 'ttalverifieduser1aws1000@guerrillamailblock.com',
                    zip: '00001'
                },
                shortDesc: `TCM-6340 API Load Testing vu${Math.floor(Math.random() * 10000)}`,
...
export function matchLegalCase(baseUrl, caseNumber, authToken) {
    ...
    try {
        ...
        check(response, {
            ...
            'Matched Case': (res) => res.json('data.matchLegalCase') !== null
        });
...
export function deleteCase(baseUrl, caseNumber, authToken) {
    ...
    try {
        ...
        check(response, {
            'Status is 200': (res) => res.status === 200,
            'Case Number Deleted': (res) => res.json('data') !== null
        });
