...
export function Peakload() {
    return {
        stages: [
            { duration: '3m', target: 100, name: 'Stage 1: 100 Users Ramp-up' }, // Ramp-up to 100 virtual users over 3 minutes
...
export function getBaseUrl(environment) {
    const environments = {
        aws: {
            baseUrl: 'https://api1.aws.legalmatch.com' //AWS environment
        }
    };
     ...
     caseNumber
        ...
        variables: {
            data: {
                ...
                shortDesc: `TCM-6242 API Load Testing vu${Math.floor(Math.random() * 10000)}`,
