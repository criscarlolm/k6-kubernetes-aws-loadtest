...
export function Peakload() {
    return {
        stages: [
            { duration: '3m', target: 1, name: 'Stage 1: 1 Users Ramp-up' }, // Ramp-up to 100 virtual users over 3 minutes
            { duration: '20m', target: 1, name: 'Stage 2: 1 Users Stay' } // Stay at 100 virtual users for 20 minutes
        ],
        thresholds: {
            //  Once baseline and threshold are defined. we will set this thresshold.
            http_req_duration: ['p(95)<3500'], // 95% of requests should complete within 2 seconds
            http_reqs: ['rate>0.7'], // Throughput should be at least 50 requests per second
            http_req_failed: ['rate<0.1'] // Error rate should be less than 0.05%
        }
    };
}
