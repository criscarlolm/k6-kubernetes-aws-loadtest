...
export function ScalabilityLoadTest() {
    return {
        stages: [
            { duration: '5m', target: 2, name: 'Stage 1: 2 Users Ramp-up' },
            { duration: '5m', target: 4, name: 'Stage 2: 4 Users' },
            { duration: '5m', target: 6, name: 'Stage 3: 6 Users' },
            { duration: '5m', target: 8, name: 'Stage 4: 8 Users' },
            { duration: '5m', target: 10, name: 'Stage 5: 10 Users' },
            { duration: '5m', target: 12, name: 'Stage 6: 12 Users' },
            { duration: '5m', target: 14, name: 'Stage 7: 14 Users' },
            { duration: '5m', target: 16, name: 'Stage 8: 16 Users' },
            { duration: '5m', target: 18, name: 'Stage 9: 18 Users' },
            { duration: '5m', target: 20, name: 'Stage 10: 20 Users' },
            { duration: '10m', target: 20, name: 'Stage 11: 20 Users (Constant)' },
        ...
        thresholds: {
            ...
            http_reqs: ['rate>20'], // Throughput should be at least 20 requests per second
            http_req_failed: ['rate<0.02'] // Error rate should be less than 0.02%
        }
    };
}
...
export function insertLegalCase(baseUrl) {
     ...
     caseNumber
        ...
        variables: {
            data: {
                ...
                shortDesc: `TCM-6304 API Load Testing vu${Math.floor(Math.random() * 10000)}`,
    ...
    try {
        ...
        check(response, {
            ...
            'Response body has data InsertCase': (res) => res.json('data') !== null,
            'Response data has caseNumber': (res) =>
                res.json('data.insertLegalCase.legalCase.caseNumber') !== null,
            'Response data has authToken': (res) =>
                res.json('data.insertLegalCase.authToken') !== null
        });
...
export function matchLegalCase(baseUrl, caseNumber, authToken) {
    ...
    try {
        ...
        check(response, {
            'Status is 200': (res) => res.status === 200,
            'Response body has data in matchLegalCase': (res) => res.json('data') !== null,
...
export function deleteCase(baseUrl, caseNumber, authToken) {
    ...
    try {
        ...
        check(response, {
            'Status is 200': (res) => res.status === 200,
            'Response body has data in Delete Case': (res) => res.json('data') !== null
        });
